...
def load_users(df: pd.DataFrame):
    ...
    logger.info(f"Loaded {len(df)} users into PostgreSQL")
...
def load_posts(df: pd.DataFrame):
    ...
    logger.info(f"Loaded {len(df)} posts into PostgreSQL")
...
def load_employees(df: pd.DataFrame):
    ...
    logger.info(f"Loaded {len(df)} employees into PostgreSQL")
